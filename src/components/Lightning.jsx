  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
          ...
          float dist = abs(uv.x);
          // Much sharper falloff for a cleaner black background
          float glow = pow(0.06 / max(0.001, dist), 2.0);
          vec3 baseColor = hsv2rgb(vec3(uHue / 360.0, 0.8, 1.0));
          vec3 col = baseColor * glow * uIntensity;
          
          // Ensure background is perfectly black away from the bolt
          col *= smoothstep(2.0, 0.0, dist);
          if (glow < 0.01) col = vec3(0.0);
